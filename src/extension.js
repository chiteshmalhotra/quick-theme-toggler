...
const QuickThemeButton = GObject.registerClass(
   class QuickThemeButton extends PanelMenu.Button {
       ...
       setupIcon() {
           this._icon = new St.Icon({ icon_name: this.getActiveIcon(), style_class: "system-status-icon" });
       ...
       iconAnimation() {
           ...
           const currentIcon = this.iconStyles[this.iconStyle] ?? this.iconStyles[0];
           this._icon.rotation_angle_z = (this.isDark ? -1 : 1) * currentIcon.rotate;
           this._icon.ease({
               rotation_angle_z: 0,
               duration: this.animationSpeed || 0,
       ...
       getActiveIcon() {
           const currentIcon = this.iconStyles[this.iconStyle] ?? this.iconStyles[0];
           return currentIcon[this.isDark ? 'dark' : 'light'];
       }
       ...
       setupSettings() {
           ...
           for (const [key, config] of Object.entries(this.settingsMap)) {
               ...
               this[propName] = this._settings[getter](key); // Ex 'icon-style' -> 'iconStyle'
